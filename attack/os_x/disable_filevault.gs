...
function Deploy() {  
    console.log("starting execution of Disable Filevault");
    // Whoami
    var myuser = user.Current();
    console.log("Our user is: "+Dump(myuser[0]));
    if (myuser[0].Username == "root") {
        var response = G.exec.ExecuteCommand("fdesetup", ["disable"]);
        ...
        console.log("Done Disable Filevault");
    } else {
